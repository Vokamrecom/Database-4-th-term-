...
go
    create  trigger TR_TEACHER_DEL 
      ...
      return;  
      go  delete TEACHER where TEACHER='Ив'
	  select * from TR_AUDIT
...
CREATE TRIGGER trigger_orders
    ...
    AS BEGIN
        ...
        FROM inserted
END
insert into  ОРДЕР(ID_ордера, Номер_счета, ID_Товара, Количество_товара, Состояние_заказа)
                   values(1,20,1,50,'уд');
...
go
    create  trigger TR_Zakaz_INS 
      on Заказ after INSERT  
      as
      declare @a1 char(10), @a2 varchar(100), @a3 char(1), @a4 char(20), @in varchar(300);
      print 'Вставка';
      set @a1 = (select Номер_счета from INSERTED);
      set @a2= (select Дата_поставки from INSERTED);
      set @a3= (select УНП from INSERTED);
	  set @a4 = (select Вид_доставки from INSERTED);
      set @in = @a1+' '+ @a2 +' '+ @a3+ ' ' +@a4;
      insert into TR_AUDIT(STMT, TRNAME, CC)  
                            values('INS', 'TR_Zakaz_INS', @in);	         
      return;  
      go

	  insert into  Заказ values(106, '2018-06-06', 110004, 116);
	  select * from TR_AUDIT
	  ...
	  go
    create  trigger TR_Zakaz_DEL 
      on Заказ after  DELETE  
      as
      declare @a1 char(10), @a2 varchar(100), @a3 char(1), @a4 char(20), @in varchar(300);
      print 'Удаление';
      set @a1 = (select Номер_счета from DELETED);
      set @a2= (select Дата_поставки from DELETED);
      set @a3= (select УНП from DELETED);
	  set @a4 = (select Вид_доставки from DELETED);
      set @in = @a1+' '+ @a2 +' '+ @a3+ ' ' +@a4;
      insert into TR_AUDIT(STMT, TRNAME, CC)  
                            values('DEL', 'TR_Zakaz_DEL', @in);	         
      return;  
      go

	   delete Заказ where Номер_счета=106
	  select * from TR_AUDIT
